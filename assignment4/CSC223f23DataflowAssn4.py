...
def generateStatisticalAnalysis(predecessor):
    ...
    for data in predecessor:
        if data is      None:  # Sentinel value indicating end of data
            yield None  # Sentinel value for this generator
            break
        
        distribution, param1, param2, count, ndarray = data
        ...
        #try:
            #mode = round(stats.mode(ndarray),0)
        #except stats.StatisticsError:
            #mode = None
        try:
...
def saveStatisticalAnalysisCSV(predecessor, csvWriter, writeHeader):
    ...
    for data in predecessor:
        ...
        csvWriter.writerow(data)  # Writing data to CSV
        yield data

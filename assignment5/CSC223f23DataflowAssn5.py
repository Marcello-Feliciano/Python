# Filename: CSC223f23DataflowAssn5.py
# ************************************************************
# Author:     Dr. Parson
# Student coauthor: Marcello Feliciano
# Major:      Game Development
# Creation Date: 11/29/2023
# Due Date: No later than 9 AM on Friday December 15.
#   This assignment is OPTIONAL and I will count it only for
#   students who have turned in assignments 1 through 4.
#   For students submitting 1 through 5, your final grade will be
#   the larger of the average of (assn1 thru assn4) and the average
#   of (assn1 thru assn5). You may skip doing assn5, and I will
#   NOT accept assn5 for anyone who has not turned in (assn1 thru assn4).
# Must be in by the above deadline in order to count because of KU
#   grading deadlines.
#   else 0%.
...
# STUDENT 1 is the same as a working STUDENT 1 to Assignment 4.
# STUDENT 1: 5% Write makeNormal(Loc, scale, size, seed)
...
def makeNormal(Loc, scale, size, seed):
    ...
    def returnNormal():
        return ('normal', Loc, scale, size, mygenerator.normal(Loc, scale, size))
    return returnNormal

# STUDENT 2 is the same as a working STUDENT 2 to Assignment 4.
# STUDENT 2: 5% Write makeExponential(scale, size, seed)
#   similar to makeUniform, that returns a closure-function
#   that returns a 5-tuple per the specification comments at the
...
def makeExponential(scale, size, seed):
    ...
    def returnExponential():
        return ('exponential', scale, None, size, mygenerator.exponential(scale, size))
    return returnExponential

# STUDENT 3 is the same as a working STUDENT 3 to Assignment 4.
# STUDENT 3: 5% Write generator generateDistribution() with these parameters
...
# See genASCII in DataflowGeneratorsDAG.py for an example
# of yielding values.
...
# STUDENT 4: 27% Write *coroutine* generateStatisticalAnalysis that iterates
# over its incoming *yield*ed value until that value == None as a
# sentinel value. Otherwise, create and YIELD a 10-tuple that consists
# of the first 4 fields of the incoming 5-tuple from yield's result:
...
# When the YIELD returns a value of None, break out of your loop,
...
# See genASCII2Count in DataflowGeneratorsDAG.py for an example
# of iterating over incoming values send()ed by __main__ and yielding values.
def generateStatisticalAnalysis():
    # Distribution, Param1, Param2, Count, Mean, Median, Mode, Pstdev, Min, Max
    # Distribution, Param1, Param2, Count come in from yield return value.
    # See DataflowGeneratorsDAG.py's genASCII2Count
    data = yield 0
    while True:
        #data = yield
        if data is None:
            ...
            break
        distribution, param1, param2, count, values = data
        mean = round(np.mean(values), 0)
        median = round(np.median(values), 0)
        try:
            mode_result = stats.mode(values)
            mode = round(mode_result.mode[0], 0)   # Extract the mode value from the result
        except (stats.StatisticsError, AttributeError):
            mode = None
        pstdev = round(stats.pstdev(values), 0)
        minimum = round(np.min(values), 0)
        maximum = round(np.max(values), 0)
        data = yield (distribution, param1, param2, count, mean, median, mode, pstdev, minimum, maximum)
    yield None

# STUDENT 5: 27% Write *coroutine* saveStatisticalAnalysisCSV that tests
...
#   'Pstdev', 'Min', 'Max']
# Then, iterate over its result from yield and writerow the incoming 10-tuple
#   to csvWriter and then YIELD that 10-tuple, re-iterating over the yielded
#   result. HOWEVER, when the incoming value from yield is None, YIELD None
#   (as a sentinel value), and fall out the bottom.
# The yield of sentinel value None at the end is new for this assignment.

# See sinkOutput2File in DataflowGeneratorsDAG.py for an example
# of iterating over a yielded value and writing and yielding values.
def saveStatisticalAnalysisCSV(csvWriter, writeHeader):
    if writeHeader:
    ...
    while True:
        ...
        data = yield data
        #coroutine = saveStatisticalAnalysisCSV(csvWriter, writeHeader)
    yield None

# Advance coroutine to first yield (header row)
    next(coroutine)

# # Iterate over results and write to CSV
    for data_to_write in received_data:
        result = coroutine.send(data_to_write)
        if result is None:
             break
__usage__ =                 \
    'USAGE: python CSC223f23DataflowAssn5.py SEED OUTFILE.csv'
...
if __name__ == '__main__':      # Entry code outside of any function.
    if len(sys.argv) != 3:
        # argv[0] is CSC223f23DataflowAssn5.py
    ...
    for distributor in (makeUniform(0, 100, samplesPerDistribution, seed),
        ...
        print("Yield count:", howManyTimesYielded)
        if howManyTimesYielded > howManyTimesToYield:
            raise RuntimeError('ERROR, Missing "yield None" as a '
                + 'sentinel value in the pipeline?')
        # Create and run 3 distinct PUSH pipeline stages.
        stage1 = generateDistribution(distributor, howManyTimesToYield)
        stage2 = generateStatisticalAnalysis()
        stage3 = saveStatisticalAnalysisCSV(outcsv, writeCSVheader)
        writeCSVheader = False
        # STUDENT 6: 31% call .__next__() on each of coroutines stage2 and
        #   stage3 to get to the upper yield statement. Then
        #   iterate over stage1's output in local variable
        #   stage1data, pass that value via send to stage2 and store its
        #   returned result in variable stage2data, pass that value via send
        #   to stage3 and store its returned result in variable stage3data,
        #   and break out the this loop when stage3data == None.
        #   See __main__ in DataflowGeneratorsDAG.py for PUSH dataflow.
        #   Note that there is NO fork in the dataflow of Assignments 4 & 5.
        ...
        for stage1data in stage1:
            stage2data = stage2.send(stage1data)
            stage3data = stage3.send(stage2data)
            
        if stage3data is None:
            continue
        break
    outfile.close()